  ...
  return (
    <div className="w-full flex flex-row gap-2 justify-center">
      {[0, 1, 2, 3, 4].map((i) => {
        ...
        if (guessed && i < guess.length) {
          tileState = stateForTile(target, letter, i);
        }
        return (
          <Tile
            key={i}
...
function stateForTile(target: string, letter: string, letterPosition: number) {
  if (!letter) return TileState.Default;
  if (target[letterPosition] === letter) return TileState.Correct;
  if (target.includes(letter)) return TileState.PartiallyCorrect;
